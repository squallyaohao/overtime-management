...
#we use utf-8 to encode our database,so make sure every element in the list should be a string and alse decoded
def sqlInsertState(table='',list=[]):
    insert_statement = 'insert into ' + table + " values('" + list[0] + "','" + list[1].decode('utf-8') + "','" + list[2].decode('utf-8') + "','" + list[3] + "','" + list[4].decode('utf-8') +"');"
    #for data in list:
        #state2 = state2 + data + "','"
    #state2 = state2[:-2] 
...
def sqlUpdateState(table='',list=''):
    state1 = 'update ' + table + ' set project="' + list[2].decode('utf-8') + '",duration=' + list[3] +',meal="' + list[4].decode('utf-8') +'" '
    state2 = 'where date="' + list[0] + '" and name="' + list[1].decode('utf-8') + '";'
